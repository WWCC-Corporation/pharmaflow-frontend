import type { SalesMetric } from '../types/sales.types'

type SalesMetricCardProps = {
  metric: SalesMetric
}

export function SalesMetricCard({ metric }: SalesMetricCardProps) {
  const Icon = metric.icon

  return (
    <article className="rounded-2xl border border-[#E8EAF3] bg-white p-5 shadow-[0_10px_30px_rgba(17,26,68,0.06)]">
      <div className="flex items-center gap-5">
        <div className={`flex h-[70px] w-[70px] shrink-0 items-center justify-center rounded-full ${metric.bubble}`}>
          <Icon size={31} />
        </div>
        <div>
